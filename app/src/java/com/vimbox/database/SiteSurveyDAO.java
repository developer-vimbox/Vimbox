...
public class SiteSurveyDAO {
    ...
    private static final String GET_SITE_SURVEYS_BY_KEYWORD = "SELECT * FROM (sitesurvey_assigned LEFT OUTER JOIN users ON users.nric = sitesurvey_assigned.ss_user) WHERE (CONCAT(last_name, ' ', first_name) LIKE ? OR lead_id like ? OR start_datetime LIKE ? OR end_datetime LIKE ? OR timeslot LIKE ?) AND status=? ORDER BY start_datetime DESC";
    ...
    public static ArrayList<SiteSurvey> getSiteSurveysByLeadIdDateTimeslot(int leadId, String date, String timeslot) {
       ...
       public static SiteSurvey  getSiteSurveysByLeadIdStartDateTimeslot(int leadId, String date, String timeslot) {
        DateTimeFormatter dtf = DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss");
        SiteSurvey results = null;
        Connection con = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                ...
                String status = rs.getString("status");
                    ...
                    results = new SiteSurvey(leadId, owner, user, address, addressTag, start, end, timeslot, remarks, status);
